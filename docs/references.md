# References

This file collects public source grounding for the repository's governance
themes. These references inform the structure of the demonstration workflow, but
the repository does not provide legal advice, certify compliance, or replace
institution-specific review.

## Equal Credit Opportunity Act and Regulation B

- Consumer Financial Protection Bureau, Regulation B, 12 CFR Part 1002:
  <https://www.consumerfinance.gov/rules-policy/regulations/1002/>
- Electronic Code of Federal Regulations, 12 CFR 1002.9, Notifications:
  <https://www.ecfr.gov/current/title-12/chapter-X/part-1002/section-1002.9>
- Consumer Financial Protection Bureau, Small Business Lending under the Equal
  Credit Opportunity Act, Regulation B:
  <https://www.consumerfinance.gov/rules-policy/final-rules/small-business-lending-under-the-equal-credit-opportunity-act-regulation-b/>

## Model Risk Management

- Board of Governors of the Federal Reserve System and Office of the Comptroller
  of the Currency, Supervisory Guidance on Model Risk Management, SR 11-7
  (April 4, 2011; issued jointly with the OCC as OCC Bulletin 2011-12):
  <https://www.federalreserve.gov/supervisionreg/srletters/sr1107.htm>

## AI Risk Management and Documentation

- National Institute of Standards and Technology, Artificial Intelligence Risk
  Management Framework:
  <https://www.nist.gov/itl/ai-risk-management-framework>
- National Institute of Standards and Technology, AI Risk Management Framework
  1.0:
  <https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10>
- National Institute of Standards and Technology, AI Resource Center:
  <https://airc.nist.gov/>

## Fair-Lending and Small Business Credit Context

- Consumer Financial Protection Bureau, Fair Lending:
  <https://www.consumerfinance.gov/fair-lending/>
- Consumer Financial Protection Bureau, Small Business Lending Collection and
  Reporting Requirements:
  <https://www.consumerfinance.gov/1071-rule/>
- Federal Financial Institutions Examination Council, Consumer Compliance
  Examination Manual:
  <https://www.ffiec.gov/resources/supervision-and-examination-materials/>

## Use Notes

- Regulation B and adverse-action materials are relevant to explanation,
  notification, and reason-review concepts.
- Model risk management guidance is relevant to validation, monitoring,
  documentation, governance, and controls.
- NIST AI RMF materials are relevant as general AI risk-management context and
  should be used carefully because they are not credit-specific legal authority.
- Fair-lending and small business lending materials are relevant to monitoring
  design and screening triggers, not automatic legal conclusions.
